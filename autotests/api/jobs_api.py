...
def get_jobs(client, start_datetime, end_datetime=None):
    ...
    return client.request('GET', routes.Routes.JOBS, params=params)
...
def get_jobs_jid_return(client, jid, count=10, cursor=None):
    ...
    return client.request('GET', routes.Routes.JOB_RETURN.format(jid), params=params)


def get_jobs_jid_returns_count(client, jid):
    return client.request('GET', routes.Routes.JOB_RETURNS_COUNT.format(jid))


def post_jobs(client, **kwargs):
    return client.request('POST', routes.Routes.JOBS, **kwargs)
